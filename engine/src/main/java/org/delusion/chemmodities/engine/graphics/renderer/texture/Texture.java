package org.delusion.chemmodities.engine.graphics.renderer.texture;

import org.apache.commons.io.IOUtils;
import org.delusion.chemmodities.engine.utils.FileUtils;
...
public class Texture {
    ...
    public Texture(String s) {
        this(s, new TextureSettings());
    }

    public Texture(String s, TextureSettings settings) {
        this.settings = settings;
        byte[] data = new byte[0];
        try {
            data = IOUtils.toByteArray(Texture.class.getClassLoader().getResourceAsStream(s));
            loadFrom(data);
        } catch (IOException e) {
            e.printStackTrace();
        }
